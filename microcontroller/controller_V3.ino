...
void currlatCallback(const std_msgs::Float32& msg)
{
  gps_latt = msg.data;
}

void currlongCallback(const std_msgs::Float32& msg)
{
  gps_longg = msg.data;
}
...
ros::Subscriber<std_msgs::Float32> curr_lat("/gps_lat", &currlatCallback);
ros::Subscriber<std_msgs::Float32> curr_long("/gps_long", &currlongCallback);
...
void setup()
{
  ...
  nh.subscribe(curr_lat);
  nh.subscribe(curr_long);
...
{  
  ...
  float tolerance = 2.0;
  float max_offset = 50;
//  Serial.println(gps_latt);
//  Serial.println(gps_longg);
  mySerial.print(gps_latt, 12);
  mySerial.print(",");
  mySerial.println(gps_longg, 12);
